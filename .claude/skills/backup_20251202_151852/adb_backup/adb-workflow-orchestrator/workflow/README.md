# adb-workflow-orchestrator Workflows

Workflow infrastructure for Workflow orchestration and execution engine

## Overview

This directory contains TOON-based workflow definitions for automating Workflow orchestration and execution engine workflows. Each workflow pairs a `.toon` file (orchestration logic) with a `.md` file (documentation).

## Workflow Index

### Implemented Workflows (3 TOON+MD pairs)

1. **[workflow-execution.md](./workflow-execution.md)** - TOON workflow execution engine
   - File: `workflow-execution.toon`
   - Parse and validate TOON files
   - Execute phases sequentially
   - Generate execution reports

2. **[bluestacks-demo.md](./bluestacks-demo.md)** - BlueStacks interactive demonstration
   - File: `bluestacks-demo.toon`
   - Interactive demo environment
   - Showcase ADB capabilities
   - Device connectivity verification

3. **[workflow-chaining.md](./workflow-chaining.md)** - Multi-workflow orchestration
...
uv run .claude/skills/adb-workflow-orchestrator/scripts/adb-run-workflow.py \
  --workflow .claude/skills/adb-workflow-orchestrator/workflow/{workflow-name}.toon \
