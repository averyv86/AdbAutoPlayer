...
class AdbController:
    ...
    d: AdbDeviceWrapper
    _unicode_input: AdbUnicodeInputManager | None

    def __init__(self):
        """Init."""
        self.d = AdbDeviceWrapper.create_from_settings()
        self._unicode_input = None  # Lazy initialization for Unicode input
    ...
    def unicode_input(self) -> AdbUnicodeInputManager:
        ...
        Returns:
            AdbUnicodeInputManager: Unicode input manager for Korean, Chinese, Japanese, etc.

        Example:
            >>> controller.unicode_input.input_korean('서초동')
        """
    ...
    def input_text_unicode(self, text: str, delay: float = 0.5) -> bool:
        ...
        This is a convenience method that uses ADBKeyboard for Unicode input.
        Requires ADBKeyboard to be installed on the device.

        Args:
            text: Text to input (supports Unicode characters).
            delay: Delay after input in seconds (default 0.5s).

        Returns:
            bool: True if input was successful, False otherwise.

        Raises:
            AutoPlayerError: If ADBKeyboard is not installed.

        Example:
            >>> controller.input_text_unicode('서초동')  # Korean
            True
            >>> controller.input_text_unicode('中文', delay=1.0)  # Chinese
            True
        """
