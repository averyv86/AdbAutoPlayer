...
| `builder-workflow` | Create single-file UV CLI scripts for existing MoAI skills |
...
**Category**: Factory (Tier 5)

**Purpose**: Create single-file UV CLI scripts for existing MoAI skills

**When to Use**:
- Need automation utilities for repetitive tasks
- Want zero-context execution (no docs reading required)
- Task fits 200-300 line single-file pattern

**Capabilities**:
- Single-file Python scripts with UV dependencies (PEP 723)
- Project-root auto-detection (works from any directory)
- Dual output modes (human-readable + `--json`)
- Automatic metadata integration

**Typical Workflow**:
1. User identifies repetitive task in existing skill
2. Alfred delegates to builder-workflow
3. Agent researches best practices via Context7
4. Generates script following IndieDevDan template
5. Updates skill YAML frontmatter with metadata
6. Creates/updates scripts-reference.md

**Example**:
```python
# User: "Add Korean docs linter to moai-workflow-docs"
Task(subagent_type="builder-workflow",
     prompt="Create lint_korean_docs.py for moai-workflow-docs skill",
     context={
         "skill_name": "moai-workflow-docs",
         "script_purpose": "Validate Korean markdown documentation",
         "input": "docs/src/",
         "output": ".moai/reports/lint-results.json"
     })
```

**Orchestration Metadata**:
- Can resume: true (single-session)
- Typical chain position: after factory-skill
- Spawns subagents: false
- Delegates to: mcp-context7 (library research)
- Execution time: ~600 seconds (10 minutes)
- Skill count: 2 (moai-foundation-core, moai-lang-unified)

**Works Well With**:
- factory-skill (create skill structure first)
- moai-foundation-core (execution rules)
- moai-lang-unified (Python best practices)

**Context Savings**: 98% reduction for automation tasks (0 tokens until script invoked)

**Script Standards** (IndieDevDan Pattern):
- 200-300 line optimal size (500 hard limit)
...
- SKILL.md lists scripts with "When to use" sections
